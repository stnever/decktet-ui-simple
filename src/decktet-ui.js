window.onload = function() {
	...
	function create() {
		...
		var deck = LangUtils.deepClone(Decktet.Cards);
		DecktetUtils.shuffle(deck);
		...
		deck.forEach(function(card) {
			...
			var frameName = DecktetUtils.cardName(card).toLowerCase() + '.png';
	...
	function updateAllButtonsState() {
		...
		buttons.forEach(function(btn) {
			// over, out, down
			var base = 'btn' + LangUtils.camelCase(btn.mode);
			if ( mode == btn.mode )
